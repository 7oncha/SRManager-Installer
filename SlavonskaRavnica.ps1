...
function Set-WindowWorkAreaBounds {
    # Borderless WPF prozor: Maximized prekriva taskbar — koristi radno podrucje
...
function Set-WindowFullscreen {
    ...
    if ($On) {
        ...
        if ($btnFullscreen) { $btnFullscreen.ToolTip = 'Vrati velicinu prozora (F11)' }
    } else {
        $window.WindowState = 'Normal'
        if ($script:WindowRestore) {
            ...
            if ($script:WindowRestore.WindowState) {
                try { $window.WindowState = $script:WindowRestore.WindowState } catch {}
            }
        }
        $script:IsFullscreen = $false
        if ($btnFullscreen) { $btnFullscreen.ToolTip = 'Maksimiziraj (taskbar ostaje vidljiv) (F11)' }
    }
    try {
...
function Get-ModDownloadUrl {
    param(
        $ModItem,
        [array]$ServerMods
    )
    if ($ModItem -and $ModItem.DownloadUrl) { return [string]$ModItem.DownloadUrl }
    if (-not $ServerMods) { $ServerMods = @(Get-ServerModList) }
    if (-not $ServerMods) { return $null }
    $lookup = if ($ModItem.ZipName) { $ModItem.ZipName } else { $ModItem.Name }
    $entry = Resolve-ServerModEntry -ServerMods $ServerMods -DisplayOrZipName $lookup
    if ($entry -and $entry.Url) { return [string]$entry.Url }
    $server = Get-ActiveServer
    if ($server -and $lookup) {
        $fname = if ($lookup -match '\.zip$') { $lookup } else { "$lookup.zip" }
        return "http://$($server.ip):$($server.webPort)/mods/$fname"
    }
    return $null
}

function Add-RecentModSyncEntry {
    param([string]$Name, [string]$Action)
    if (-not $Name) { return }
    try {
        $list = @()
        if ($script:Config.recentModSync) { $list = @($script:Config.recentModSync) }
        $entry = [PSCustomObject]@{
            name   = $Name
            action = $Action
            at     = (Get-Date).ToString('dd.MM.yyyy HH:mm')
        }
        $list = ,$entry + @($list | Where-Object { $_.name -ne $Name })
...
function Update-DashboardSyncPanel {
    if (-not $txtLastSync) { return }
    $when = if ($script:Config.lastSync) { $script:Config.lastSync } else { 'nikad' }
    $txtLastSync.Text = "Zadnji sync: $when"
    if (-not $lstRecentSyncMods) { return }
    $items = New-Object System.Collections.ArrayList
    if ($script:Config.recentModSync) {
        foreach ($r in @($script:Config.recentModSync | Select-Object -First 5)) {
            $act = if ($r.action) { $r.action } else { 'sync' }
            $nm = if ($r.name) { $r.name } else { '?' }
            $at = if ($r.at) { $r.at } else { '' }
    ...
    if ($items.Count -eq 0) {
        try {
            $since = if ($script:Config.lastLaunchAt) { [string]$script:Config.lastLaunchAt } else { $null }
            $changes = @(Get-ModChangesSinceFromBot -SinceIso $since)
            foreach ($c in @($changes | Select-Object -First 5)) {
                $nm = if ($c.name) { $c.name } elseif ($c.fileName) { $c.fileName } else { 'mod' }
                $typ = if ($c.type) { $c.type } else { 'promjena' }
                $lbl = switch ($typ) {
                    'added'   { 'dodan' }
                    'updated' { 'azuriran' }
                    'removed' { 'uklonjen' }
...
function Test-ModsFolderEmptyWarning {
    $eff = Get-EffectiveModsPath
    if (-not $eff) {
        return 'Mods folder nije postavljen - Postavke ili Multiplayer folderi.'
    }
    if (-not (Test-Path $eff)) {
        return "Folder ne postoji: $eff"
    }
    $n = @(Get-ChildItem $eff -Filter '*.zip' -ErrorAction SilentlyContinue).Count
    if ($n -eq 0) {
        return "Mods folder je prazan (0 ZIP): $eff - sync ce skinuti sve server modove."
    }
    return $null
}

function Update-ModsPathWarningBanner {
    if (-not $borderModsPathWarn) { return }
    $msg = Test-ModsFolderEmptyWarning
    if ($msg) {
        if ($txtModsPathWarn) { $txtModsPathWarn.Text = $msg }
        $borderModsPathWarn.Visibility = 'Visible'
    } else {
        $borderModsPathWarn.Visibility = 'Collapsed'
    }
}

function Refresh-ModProfilesUi {
    if (-not $cmbModProfiles) { return }
...
function Refresh-MpFolderModsGrid {
    if (-not $lstMpFolderMods) { return }
    $eff = Get-EffectiveModsPath
    if ($cmbMpFolderSelect -and $cmbMpFolderSelect.SelectedItem -and $cmbMpFolderSelect.SelectedItem.Path) {
        $p = [string]$cmbMpFolderSelect.SelectedItem.Path
        if ($p) { $eff = $p }
    }
    $items = New-Object System.Collections.ArrayList
    if ($eff -and (Test-Path $eff)) {
        foreach ($z in @(Get-ChildItem $eff -Filter '*.zip' -ErrorAction SilentlyContinue | Sort-Object Name)) {
            [void]$items.Add([PSCustomObject]@{
                Name = $z.BaseName
                Size = '{0:N1} MB' -f ($z.Length / 1MB)
            })
        }
    }
    $lstMpFolderMods.ItemsSource = $null
    $lstMpFolderMods.ItemsSource = @($items.ToArray())
}
...
function Start-ModThumbnailLoads {
    ...
    try {
        if (-not (Test-Path $script:ModThumbsPath)) {
...
function Format-MpFolderDisplayLabel {
    param([string]$Label, [string]$Path)
    $name = if ($Label) { $Label.Trim() } else { 'Folder' }
    if (-not $Path) { return $name }
    $p = [string]$Path.Trim()
    if (-not $p) { return $name }
    if ($p.Length -gt 72) {
        $p = '...' + $p.Substring($p.Length - 69)
    }
    return "$name - $p"
}

function Repair-EmptyServerMpFolderPath {
    param($Store, [string]$ServerFolderId, [string]$MainPath)
    if (-not $Store -or -not $ServerFolderId -or -not $MainPath) { return $false }
    if (-not (Test-Path $MainPath)) { return $false }
    $rec = Get-MpFolderRecord -FolderId $ServerFolderId -Store $Store
    if (-not $rec -or [bool]$rec.useMainLibrary) { return $false }
    ...
    $isAutoEmpty = ($path -match 'SR-Mods-')
    if (-not $isAutoEmpty) { return $false }
    $zipHere = 0
    if (Test-Path $path) {
        $zipHere = @(Get-ChildItem $path -Filter "*.zip" -ErrorAction SilentlyContinue).Count
    }
    if ($zipHere -gt 0) { return $false }
    $zipMain = @(Get-ChildItem $MainPath -Filter "*.zip" -ErrorAction SilentlyContinue).Count
    if ($zipMain -le 0) { return $false }
    $rec.path = $MainPath
    $rec.updatedAt = (Get-Date).ToString("o")
    Write-Log "MP folder: prazan SR-Mods zamijenjen glavnom bibliotekom ($MainPath)"
    return $true
}

function Invoke-MpFolderActivated {
    param([string]$Reason = '')
    Sync-ActiveModsPathCache | Out-Null
    $eff = Get-EffectiveModsPath
    if ($eff -and -not (Test-Path $eff)) {
        try { New-Item -ItemType Directory -Path $eff -Force | Out-Null } catch {}
    }
    $n = 0
    if ($eff -and (Test-Path $eff)) {
        $n = @(Get-ChildItem $eff -Filter "*.zip" -ErrorAction SilentlyContinue).Count
    }
    if ($txtMyModCount) { $txtMyModCount.Text = "$n" }
...
function Sync-ActiveModsPathCache {
    $p = $null
    $active = Get-ActiveMultiplayerFolder
    if ($active -and $active.Path) { $p = [string]$active.Path.Trim() }
    if (-not $p -and $script:Config.modsPath) { $p = [string]$script:Config.modsPath.Trim() }
    $script:ActiveModsPath = if ($p) { $p } else { "" }
    return $script:ActiveModsPath
}
...
function Ensure-ServerMpFolder {
    ...
    $repaired = Repair-EmptyServerMpFolderPath -Store $store -ServerFolderId $id -MainPath $main
    if ($repaired) {
    ...
    if (-not $folders.ContainsKey($id)) {
        ...
        if (-not $slug) { $slug = "Server$($script:Config.activeServer)" }
        # Preferiraj glavnu biblioteku (Documents\...\mods) umjesto praznog SR-Mods-* foldera
        $path = if ($main -and (Test-Path $main)) { $main } else { Join-Path $parent "SR-Mods-$slug" }
    ...
    if ($store.folders) {
        $store.folders.PSObject.Properties | ForEach-Object {
            ...
            [void]$items.Add([PSCustomObject]@{
                ...
                Label          = $label
                Path           = [string]$path
                DisplayLabel   = (Format-MpFolderDisplayLabel -Label $label -Path $path)
...
function Get-EffectiveModsPath {
    ...
    if ($active -and $active.Path) {
        ...
        if ($p) {
            $script:ActiveModsPath = $p
            return $p
        }
    }
    $fallback = if ($script:Config.modsPath) { [string]$script:Config.modsPath } else { "" }
    $script:ActiveModsPath = $fallback
    return $fallback
}
...
function Reset-MpFolderToMainLibrary {
    ...
    Save-MultiplayerFoldersStore $store
    Invoke-MpFolderActivated -Reason 'glavna biblioteka'
}
...
function Refresh-MpFoldersPage {
    ...
    if ($cmbMpFolderSelect) {
        ...
        $cmbMpFolderSelect.DisplayMemberPath = "DisplayLabel"
    ...
    Refresh-ModProfilesUi
    Refresh-MpFolderModsGrid
}
...
function Sync-MpFolderQuickPick {
    ...
    $cmbMpFolderQuick.DisplayMemberPath = "DisplayLabel"
    $active = Get-ActiveMultiplayerFolder
    $script:MpFolderComboUpdating = $true
    try {
        if ($active) {
...
function Normalize-LauncherVersion {
    param([string]$Version)
...
function Check-ForUpdate {
    ...
    try {
        ...
        if ($api) {
            ...
            if ($latest -and $latest.version -and (Test-LauncherUpdateAvailable $latest.version)) {
                $ver = Normalize-LauncherVersion $latest.version
                $tag = "v$ver"
                Apply-ZipToLatestVersion -Version $ver -Notes $latest.notes -TagName $tag
    ...
    try {
        ...
        $remoteVersion = Normalize-LauncherVersion $release.tag_name
        if ($remoteVersion -and (Test-LauncherUpdateAvailable $remoteVersion)) {
            $tag = if ($release.tag_name -match '^v') { $release.tag_name } else { "v$remoteVersion" }
            Apply-ZipToLatestVersion -Version $remoteVersion -Notes $release.body -ReleaseAssets $release.assets -TagName $tag
...
function Download-Update {
    ...
    $normTarget = Normalize-LauncherVersion $targetVer
    $normInstalled = Normalize-LauncherVersion $installedVer
    if ($normTarget -and $normInstalled -and $normInstalled -ne $normTarget) {
        Write-Log "UPOZORENJE: ocekivano v$normTarget, u ps1 je v$normInstalled - provjeri GitHub release ZIP."
    }

    try { Repair-SRManagerDesktopShortcut | Out-Null } catch {}

    $showVer = if ($normInstalled) { $normInstalled } else { $normTarget }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Window.Resources>
        ...
        <SolidColorBrush x:Key="BodyText" Color="#eeeeee"/>
        <SolidColorBrush x:Key="CardBorder" Color="#3a2e10"/>
        <SolidColorBrush x:Key="CardBg" Color="#161616"/>
        <SolidColorBrush x:Key="WarnOrange" Color="#F0A020"/>
        <SolidColorBrush x:Key="BgDeep" Color="#0a0a0b"/>
        <SolidColorBrush x:Key="BgPanel" Color="#111111"/>
        <SolidColorBrush x:Key="BgElevated" Color="#161616"/>
        <SolidColorBrush x:Key="BorderSubtle" Color="#222222"/>

        <!-- Jedinstvena kartica (16px grid, farmbuddy dashboard) -->
        <Style x:Key="SrCard" TargetType="Border">
        ...
        <Style x:Key="SrCardHero" TargetType="Border" BasedOn="{StaticResource SrCard}">
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="TextBox">
                        ...
                        <ControlTemplate.Triggers>
                            <Trigger Property="IsMouseOver" Value="True">
                                <Setter TargetName="bd" Property="BorderBrush" Value="#4a3a18"/>
                            </Trigger>
        ...
        <Style x:Key="ModernTextBox" TargetType="TextBox" BasedOn="{StaticResource DarkInput}"/>
        ...
        <Style x:Key="ModernPasswordBox" TargetType="PasswordBox" BasedOn="{StaticResource DarkPasswordBox}"/>
        ...
        <!-- Tamni CheckBox (bez bijele pozadine) -->
                    ...
                    <ControlTemplate TargetType="CheckBox">
                        <StackPanel Orientation="Horizontal">
                            <Border x:Name="box" Width="18" Height="18" CornerRadius="4"
                                    Background="#141414" BorderBrush="#3a2e10" BorderThickness="1"
                                    Margin="0,0,10,0" VerticalAlignment="Center">
                                <TextBlock x:Name="tick" Text="&#x2713;" FontSize="12" FontWeight="Bold"
                        ...
                        <ControlTemplate.Triggers>
                            <Trigger Property="IsChecked" Value="True">
                                <Setter TargetName="box" Property="Background" Value="#1a1408"/>
                                <Setter TargetName="box" Property="BorderBrush" Value="{StaticResource Gold}"/>
                                <Setter TargetName="tick" Property="Visibility" Value="Visible"/>
                            </Trigger>
                            <Trigger Property="IsMouseOver" Value="True">
                                <Setter TargetName="box" Property="BorderBrush" Value="{StaticResource GoldDim}"/>
                            </Trigger>
                            <Trigger Property="IsEnabled" Value="False">
        ...
        <!-- Platform odabir (kartice umjesto obicnih filter gumba) -->
        <Style x:Key="PlatformCard" TargetType="RadioButton">
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="RadioButton">
                        <Border x:Name="bd" Background="#121212" CornerRadius="10"
                                BorderBrush="#2a2a2a" BorderThickness="1" Padding="14,12">
                            <ContentPresenter HorizontalAlignment="Left" VerticalAlignment="Center"/>
                        </Border>
                        <ControlTemplate.Triggers>
                            <Trigger Property="IsChecked" Value="True">
        ...
        <!-- Tamni ScrollBar (ScrollViewer u listama/stranicama) -->
                                    ...
                                    <Thumb>
                                        <Thumb.Template>
                                            <ControlTemplate TargetType="Thumb">
                                                <Border x:Name="thumbBd" Background="#333" CornerRadius="4"
                                                        Margin="2" Opacity="0.6"/>
                                                <ControlTemplate.Triggers>
                                                    <Trigger Property="IsMouseOver" Value="True">
                                                        <Setter TargetName="thumbBd" Property="Background" Value="#F5C518"/>
                                                        <Setter TargetName="thumbBd" Property="Opacity" Value="0.85"/>
                                                    </Trigger>
                                                </ControlTemplate.Triggers>
                                            </ControlTemplate>
                                        </Thumb.Template>
                    ...
                    <ControlTemplate TargetType="RadioButton">
                        <Border x:Name="bd" Background="{TemplateBinding Background}"
                            ...
                            <Grid>
                                ...
                                <Border x:Name="indicator" Width="4" CornerRadius="2"
        ...
        <Style x:Key="NavSection" TargetType="TextBlock">
            ...
            <Setter Property="Foreground" Value="{StaticResource MutedText}"/>
            <Setter Property="FontFamily" Value="Segoe UI"/>
            <Setter Property="Margin" Value="12,14,0,6"/>
        </Style>
        ...
        <!-- Tamni ComboBox (dropdown + odabrani tekst na dark temi) -->
                ...
                <Setter.Value>
                    <ControlTemplate TargetType="ComboBoxItem">
                        ...
                        <ControlTemplate.Triggers>
                            ...
                            <Trigger Property="IsSelected" Value="True">
                                <Setter TargetName="bd" Property="Background" Value="#2a2208"/>
                                <Setter Property="Foreground" Value="#F5C518"/>
                            </Trigger>
                        </ControlTemplate.Triggers>
                    </ControlTemplate>
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="ComboBox">
                        <Grid>
                            <ToggleButton x:Name="toggle" Grid.Column="2"
                                            Focusable="False" IsChecked="{Binding IsDropDownOpen, Mode=TwoWay, RelativeSource={RelativeSource TemplatedParent}}"
                                            ClickMode="Press" Background="#141414" BorderBrush="{StaticResource CardBorder}" BorderThickness="1">
                                <ToggleButton.Template>
                                    <ControlTemplate TargetType="ToggleButton">
                                        <Border x:Name="cbBd" Background="#141414"
                                            ...
                                            <Grid>
                                                ...
                                                <ContentPresenter x:Name="cbContent" Margin="10,6,4,6"
                                                                  VerticalAlignment="Center" HorizontalAlignment="Left"/>
                                                <Path Grid.Column="1" Data="M 0 0 L 4 4 L 8 0 Z" Fill="#888"
                                                      HorizontalAlignment="Center" VerticalAlignment="Center"/>
                                            </Grid>
                                        </Border>
                                        <ControlTemplate.Triggers>
                                            <Trigger Property="IsMouseOver" Value="True">
                                                <Setter TargetName="cbBd" Property="BorderBrush" Value="#4a3a18"/>
                                            </Trigger>
                                            <Trigger Property="IsChecked" Value="True">
                                                <Setter TargetName="cbBd" Property="BorderBrush" Value="{StaticResource Gold}"/>
                                            </Trigger>
                                        </ControlTemplate.Triggers>
                                    </ControlTemplate>
                                </ToggleButton.Template>
                            </ToggleButton>
                            <ContentPresenter x:Name="selectionBox" IsHitTestVisible="False"
                                              Margin="10,6,32,6" VerticalAlignment="Center" HorizontalAlignment="Left"
                                              Content="{TemplateBinding SelectionBoxItem}"
                                              ContentTemplate="{TemplateBinding SelectionBoxItemTemplate}"
                                              ContentTemplateSelector="{TemplateBinding ItemTemplateSelector}"
                                              TextBlock.Foreground="{StaticResource BodyText}"/>
                            <Popup x:Name="popup" Placement="Bottom" IsOpen="{TemplateBinding IsDropDownOpen}"
                                   AllowsTransparency="True" Focusable="False" PopupAnimation="Slide">
                                <Border Background="#141414" BorderBrush="{StaticResource CardBorder}" BorderThickness="1" CornerRadius="8"
        ...
        <Grid>
            ...
            <Grid Grid.Row="1">
                    ...
                    <DockPanel>
                        ...
                        <StackPanel DockPanel.Dock="Top" Margin="8,0">
                            ...
                            <RadioButton x:Name="navMpFolders" Content="Multiplayer folderi"
                                         Style="{StaticResource NavBtn}" GroupName="nav"
                                         ToolTip="Dedicated mod folderi po serveru, favoriti, brzi odabir pri joinu"/>
                ...
                <Grid Grid.Column="1" Background="#111">
                    <!-- SR siluete: traktor + psenica + blagi gold glow -->
                    <Canvas IsHitTestVisible="False" ClipToBounds="True">
                        <Ellipse Width="420" Height="420" Canvas.Right="-80" Canvas.Bottom="-100" Opacity="0.06">
                            <Ellipse.Fill>
                                <RadialGradientBrush>
                                    <GradientStop Color="#F5C518" Offset="0"/>
                                    <GradientStop Color="#00F5C518" Offset="1"/>
                        ...
                        <Path Canvas.Left="60" Canvas.Top="100" Fill="#F5C518" Opacity="0.028"
                              Data="M 120 200 L 160 120 L 200 120 L 240 80 L 320 80 L 360 120 L 400 120 L 400 200 L 380 200 A 40 40 0 1 1 300 200 L 220 200 A 40 40 0 1 1 140 200 Z M 310 200 A 30 30 0 1 0 370 200 A 30 30 0 1 0 310 200 Z M 150 200 A 30 30 0 1 0 210 200 A 30 30 0 1 0 150 200 Z M 250 100 L 250 140 L 350 140 L 350 100 Z"/>
                        <Path Canvas.Left="60" Canvas.Top="100" Fill="#F5C518" Opacity="0.022"
                              Data="M 100 160 L 120 160 L 120 120 L 160 120 L 160 100 L 100 100 Z"/>
                        <Path Canvas.Left="60" Canvas.Top="270" Fill="#F5C518" Opacity="0.02"
                              Data="M 60 20 L 440 20 L 440 30 L 60 30 Z M 80 30 L 80 50 L 100 50 L 100 30 Z M 200 30 L 200 50 L 220 50 L 220 30 Z M 320 30 L 320 50 L 340 50 L 340 30 Z M 400 30 L 400 50 L 420 50 L 420 30 Z"/>
                        <Path Canvas.Left="520" Canvas.Top="380" Stroke="#F5C518" StrokeThickness="2" Opacity="0.035"
                              Data="M 0 80 L 0 0 M 0 45 L -22 28 M 0 38 L 24 20 M 0 30 L -14 12 M 0 22 L 16 6"/>
                        <Path Canvas.Left="560" Canvas.Top="360" Stroke="#F5C518" StrokeThickness="2" Opacity="0.03"
                              Data="M 0 95 L 0 0 M 0 52 L -26 32 M 0 42 L 28 24 M 0 34 L -16 14 M 0 26 L 18 8"/>
                        <Path Canvas.Left="600" Canvas.Top="400" Stroke="#BF9B0F" StrokeThickness="1.5" Opacity="0.025"
                              Data="M 0 70 L 0 0 M 0 40 L -18 24 M 0 32 L 20 16"/>
                        <Path Canvas.Right="120" Canvas.Top="60" Fill="#F5C518" Opacity="0.015"
                              Data="M 0 0 L 80 0 L 100 40 L 60 80 L 20 60 Z"/>
                    </Canvas>
                    ...
                    <ScrollViewer x:Name="pageDash" VerticalScrollBarVisibility="Auto">
                        <StackPanel Margin="24,24,24,24">
                            <Grid Margin="0,0,0,16">
                                <StackPanel>
                                    <TextBlock Text="Dashboard" Style="{StaticResource PageTitle}"/>
                                    ...
                                    <Border Grid.Column="1" Background="#0a0a0a" CornerRadius="0,14,14,0"
                                        ...
                                        <StackPanel VerticalAlignment="Center">
                                            ...
                                            <Grid Margin="0,0,0,12">
                                                ...
                                                <TextBlock Grid.Row="0" Text="MOD FOLDER" Style="{StaticResource SectionLabel}"
                                                           Margin="0,0,0,6"/>
                                                <ComboBox Grid.Row="1" x:Name="cmbMpFolderQuick"
                            ...
                            <Border x:Name="borderModsPathWarn" Visibility="Collapsed"
                                    ...
                                    Margin="0,0,0,12" BorderBrush="#E5484D" BorderThickness="1">
                                <TextBlock x:Name="txtModsPathWarn" Text="" FontSize="11"
                    ...
                    <!-- PAGE: MODOVI -->
                    <Grid x:Name="pageMods" Visibility="Collapsed" Margin="24,24,24,24">
                        <Grid.RowDefinitions>
                            <RowDefinition Height="Auto"/>
                            <RowDefinition Height="Auto"/>
                            <RowDefinition Height="Auto"/>
                        ...
                        <Grid Grid.Row="0" Margin="0,0,0,16">
                            <StackPanel>
                                <TextBlock Text="Upravitelj modova" Style="{StaticResource PageTitle}"/>
                                ...
                                <ListBox x:Name="lstMods" Background="Transparent" Foreground="#ddd"
                                                ...
                                                <Border.Style>
                                                    <Style TargetType="Border">
                                                        <Style.Triggers>
                                                            ...
                                                            <Trigger Property="IsMouseOver" Value="True">
                                                                <Setter Property="BorderBrush" Value="{StaticResource CardBorder}"/>
                                                                <Setter Property="Background" Value="#1a1810"/>
                                                            </Trigger>
                                                        </Style.Triggers>
                                                    </Style>
                                                </Border.Style>
                                                                    ...
                                                                    <TextBlock.Style>
                                                                        <Style TargetType="TextBlock">
                                                                            ...
                                                                            <Style.Triggers>
                                                                                ...
                                                                                <DataTrigger Binding="{Binding Status}" Value="ZASTARIO">
                                                                                    <Setter Property="Foreground" Value="{StaticResource WarnOrange}"/>
                                                                                </DataTrigger>
                                                                                <DataTrigger Binding="{Binding Status}" Value="EXTRA">
                                    ...
                                    <ListBox.ContextMenu>
                                        <ContextMenu Background="#161616" Foreground="#eee" BorderBrush="#333">
                                            <MenuItem x:Name="ctxCopyDownloadLink" Header="Kopiraj link za rucni download"/>
                                            <Separator/>
                    ...
                    <ScrollViewer x:Name="pageMpFolders" Visibility="Collapsed"
                        ...
                        <StackPanel Margin="24,24,24,24" MaxWidth="960">
                            <Grid Margin="0,0,0,18">
                                <StackPanel>
                                    <TextBlock Text="Multiplayer folderi" Style="{StaticResource PageTitle}"/>
                            ...
                            <Border Background="#161616" CornerRadius="10" Padding="20"
                                ...
                                <StackPanel>
                                    ...
                                    <TextBlock Text="JSON u AppData — aktivni profil za sync mod foldera"
                                               FontSize="11" Foreground="#666" FontFamily="Segoe UI"
                                               TextWrapping="Wrap" Margin="0,0,0,10"/>
                                    <ComboBox x:Name="cmbModProfiles" Style="{StaticResource DarkComboBox}"/>
                            ...
                            <Border Background="#111" CornerRadius="10" Padding="20"
                                ...
                                <StackPanel>
                                    ...
                                    <ListBox x:Name="lstMpFolderMods" Background="Transparent" BorderThickness="0"
                                             Foreground="#ccc" FontFamily="Segoe UI" MaxHeight="200">
                                        <ListBox.ItemTemplate>
                                            <DataTemplate>
                                                <Grid Margin="0,2">
                                                    <Grid.ColumnDefinitions>
                                                    ...
                                                    <TextBlock Grid.Column="1" Text="{Binding Size}" FontSize="10"
                                                               Foreground="#666" Margin="12,0,0,0"/>
                                                </Grid>
                                            </DataTemplate>
                                        </ListBox.ItemTemplate>
                                    </ListBox>
                            ...
                            <Grid Margin="0,16,0,0">
                                ...
                                <TextBlock Grid.Column="0" VerticalAlignment="Center" FontSize="11"
                                           ...
                                           Text="Primijeni odabir prije synca. Aktiviraj i pokreni FS25 = isti flow kao Udi na server."/>
                    ...
                    <ScrollViewer x:Name="pageSettings" Visibility="Collapsed"
                                  VerticalScrollBarVisibility="Auto">
                        <StackPanel Margin="24,24,24,24">
                            <TextBlock Text="Postavke" Style="{StaticResource PageTitle}" Margin="0,0,0,16"/>
                            ...
                            <Border Background="#161616" CornerRadius="10" Padding="24"
                                ...
                                <StackPanel>
                                    ...
                                    <WrapPanel x:Name="platformPanel" Margin="0,0,0,16">
                                        <RadioButton x:Name="platformSteam" GroupName="gamePlatform"
                                                     Style="{StaticResource PlatformCard}" Tag="steam" IsChecked="True">
                                            <StackPanel>
                                                <TextBlock Text="Steam" FontSize="14" FontWeight="SemiBold" Foreground="{Binding Foreground, RelativeSource={RelativeSource AncestorType=RadioButton}}"/>
                                                <TextBlock Text="PC biblioteka" FontSize="10" Foreground="#666" Margin="0,4,0,0"/>
                                            </StackPanel>
                                        </RadioButton>
                                        <RadioButton x:Name="platformGiants" GroupName="gamePlatform"
                                                     Style="{StaticResource PlatformCard}" Tag="giants">
                                            <StackPanel>
                                                <TextBlock Text="Giants" FontSize="14" FontWeight="SemiBold" Foreground="{Binding Foreground, RelativeSource={RelativeSource AncestorType=RadioButton}}"/>
                                                <TextBlock Text="Installer / direktno" FontSize="10" Foreground="#666" Margin="0,4,0,0"/>
                                            </StackPanel>
                                        </RadioButton>
                                        <RadioButton x:Name="platformXbox" GroupName="gamePlatform"
                                                     Style="{StaticResource PlatformCard}" Tag="xbox">
                                            <StackPanel>
                                                <TextBlock Text="Xbox" FontSize="14" FontWeight="SemiBold" Foreground="{Binding Foreground, RelativeSource={RelativeSource AncestorType=RadioButton}}"/>
                                                <TextBlock Text="Game Pass / MS Store" FontSize="10" Foreground="#666" Margin="0,4,0,0"/>
                                            </StackPanel>
                                        </RadioButton>
                                    </WrapPanel>
                                    ...
                                    <Grid Margin="0,0,0,14">
                                        ...
                                        <StackPanel Grid.Column="0">
                                            <TextBlock Text="Maximiziraj prozor pri pokretanju" FontSize="13"
                                                       Foreground="#ddd" FontFamily="Segoe UI"/>
                                            <TextBlock Text="Launcher preko radnog podrucja — taskbar ostaje vidljiv (F11)"
                                                       FontSize="11" Foreground="#666" FontFamily="Segoe UI"
                                                       Margin="0,2,0,0"/>
                                        </StackPanel>
...
try {
    ...
    if ($preloadMods -and (Test-Path $preloadMods)) {
        $script:PreloadedLocalModCount = @(Get-ChildItem $preloadMods -Filter "*.zip" -ErrorAction SilentlyContinue).Count
...
$pageMpFolders       = $window.FindName("pageMpFolders")
...
$lstRecentSyncMods   = $window.FindName("lstRecentSyncMods")
$borderModsPathWarn  = $window.FindName("borderModsPathWarn")
$txtModsPathWarn     = $window.FindName("txtModsPathWarn")
$lstMpFolderMods     = $window.FindName("lstMpFolderMods")
...
function Apply-SelectedMpFolder {
    ...
    if ($rec) {
        ...
        if ($Item.Path) { $rec.path = [string]$Item.Path }
        $rec.updatedAt = (Get-Date).ToString("o")
    }
    Set-ActiveMpFolderForServer $Item.Id
    Save-MultiplayerFoldersStore $store
    Invoke-MpFolderActivated -Reason $Item.Label
    $script:MpFolderComboUpdating = $true
    try { Refresh-MpFoldersPage } finally { $script:MpFolderComboUpdating = $false }
}
...
if ($cmbMpFolderSelect) {
    $cmbMpFolderSelect.Add_SelectionChanged({
        ...
        if ($item) {
            ...
            if ($txtMpFolderPath -and $item.Path) { $txtMpFolderPath.Text = [string]$item.Path }
            Refresh-MpFolderModsGrid
        }
    })
}
if ($lstMpFolders) {
    $lstMpFolders.Add_SelectionChanged({
        $sel = $lstMpFolders.SelectedItem
        if ($sel -and $cmbMpFolderSelect) {
            $script:MpFolderComboUpdating = $true
            try { $cmbMpFolderSelect.SelectedItem = $sel } finally { $script:MpFolderComboUpdating = $false }
            Refresh-MpFolderModsGrid
        }
    })
}
...
if ($btnMpCreateFolder) {
    $btnMpCreateFolder.Add_Click({
        ...
        $rec.updatedAt = (Get-Date).ToString("o")
        Save-MultiplayerFoldersStore $store
        $item = (Get-MpFolderList | Where-Object { $_.Id -eq $id } | Select-Object -First 1)
        if ($item) { Apply-SelectedMpFolder $item } else { Refresh-MpFoldersPage }
...
if ($btnMpAddFolder) {
    $btnMpAddFolder.Add_Click({
        ...
        $item = (Get-MpFolderList | Where-Object { $_.Id -eq $id } | Select-Object -First 1)
        if ($item) { Apply-SelectedMpFolder $item } else { Refresh-MpFoldersPage }
        try { Show-Toast "Folder dodan: $(Split-Path $path -Leaf)" "success" } catch {}
    })
}
...
function Show-SaveWizard {
    ...
    $txtSaveInfo.FontFamily = New-Object System.Windows.Media.FontFamily('Segoe UI')
    $txtSaveInfo.Visibility = 'Collapsed'
    ...
    $txtSummary.Visibility = 'Collapsed'

    $contentHost.Children.Add($lstSaves) | Out-Null
    $contentHost.Children.Add($txtMods) | Out-Null
    $contentHost.Children.Add($txtSaveInfo) | Out-Null
    $contentHost.Children.Add($chkWizardSkipIntro) | Out-Null
    $contentHost.Children.Add($lstProfiles) | Out-Null
    $contentHost.Children.Add($chkBackup) | Out-Null
    $contentHost.Children.Add($txtSummary) | Out-Null
    ...
    $btnBrowseMods.Content = 'Odaberi folder...'
    ...
    $sp.Children.Add($btnBrowseMods) | Out-Null
    $sp.Children.Add($bp) | Out-Null
    $outer.Child = $sp
    $dlg.Content = $outer
    ...
    $state = @{
        ...
        SkipGiantsIntro = -not [bool]$gs.introScene
    }
    $chkWizardSkipIntro.IsChecked = $state.SkipGiantsIntro

    $btnBrowseMods.Add_Click({
        $fb = New-Object System.Windows.Forms.FolderBrowserDialog
        $fb.Description = 'Odaberi FS25 mods folder'
        if ($txtMods.Text) { $fb.SelectedPath = $txtMods.Text }
        if ($fb.ShowDialog() -eq 'OK') { $txtMods.Text = $fb.SelectedPath }
    })
    ...
    $render = {
        ...
        $btnNext.Content = if ($s -ge 6) { 'Primijeni' } else { 'Dalje' }
        switch ($s) {
            1 {
                $body.Text = 'Korak 1: Odaberi FS25 savegame slot koji konfiguriras:'
                $lstSaves.Visibility = 'Visible'
                $items = @()
                foreach ($sg in @(Get-FS25SavegameList)) {
                    $label = if ($sg.Name) { $sg.Name } else { "Slot $($sg.Slot)" }
                    $items += [PSCustomObject]@{
                        DisplayLine = "Slot $($sg.Slot): $label | $($sg.MapTitle) | $($sg.SizeMb) MB | $($sg.LastWrite)"
                        SaveInfo    = $sg
                    }
                }
            ...
            2 {
                $body.Text = 'Korak 2: Mods folder za ovaj save (sinkronizira se u gameSettings.xml):'
                $txtMods.Visibility = 'Visible'
                $btnBrowseMods.Visibility = 'Visible'
                $txtMods.Text = $state.ModsPath
            }
            3 {
                $body.Text = 'Korak 3: Pregled savea i Giants intro:'
                $txtSaveInfo.Visibility = 'Visible'
                $chkWizardSkipIntro.Visibility = 'Visible'
                if ($state.SelectedSave) {
                    $sg = $state.SelectedSave
                    $txtSaveInfo.Text = @(
                        "Slot: $($sg.Slot)"
                        "Naziv: $($sg.Name)"
                        "Mapa: $($sg.MapTitle)"
                    ...
                    $txtSaveInfo.Text = '(nema odabranog savea)'
                }
            }
            4 {
                $body.Text = 'Korak 4: Mod profil (mod-profiles.json) — povezi loadout s ovim saveom:'
                $lstProfiles.Visibility = 'Visible'
                Ensure-ServerModProfile | Out-Null
                $profiles = @(Get-ModProfileList)
                ...
                if ($state.ProfileId) {
                    $sel = $profiles | Where-Object { $_.Id -eq $state.ProfileId } | Select-Object -First 1
                    if ($sel) { $lstProfiles.SelectedItem = $sel }
                } elseif ($profiles.Count -gt 0) {
                    $lstProfiles.SelectedIndex = 0
                }
            }
            5 {
                $body.Text = 'Korak 5: Sigurnosna kopija prije promjena:'
                $chkBackup.Visibility = 'Visible'
            }
            6 {
                $body.Text = 'Korak 6: Potvrda — ovo ce se primijeniti:'
                $txtSummary.Visibility = 'Visible'
                $sel = if ($state.SelectedSave) { "Slot $($state.SelectedSave.Slot) - $($state.SelectedSave.Name)" } else { '(nije odabran)' }
                $intro = if ($chkWizardSkipIntro.IsChecked) { 'preskocen (isIntroActive=false)' } else { 'ukljucen' }
                $bakTxt = if ($chkBackup.IsChecked) { 'da' } else { 'ne' }
                $txtSummary.Text = @(
                    "Save: $sel"
                    "Mods folder: $($state.ModsPath)"
                    "Mod profil: $($state.ProfileLabel)"
                    "Giants intro: $intro"
                    "Backup savea: $bakTxt"
                    "gameSettings.xml: modsDirectoryOverride + isIntroActive"
                ) -join "`n"
            }
        }
    }.GetNewClosure()
    ...
    $applyWizard = {
        ...
        if ($state.ModsPath) {
            if (-not (Test-Path $state.ModsPath)) {
                New-Item -ItemType Directory -Path $state.ModsPath -Force | Out-Null
            }
            $script:Config.modsPath = $state.ModsPath
            if ($txtModsPath) { $txtModsPath.Text = $state.ModsPath }
            Update-ModsDirectoryOverride $state.ModsPath | Out-Null
        }
        if ($state.ProfileId) {
            $store = Get-ModProfilesStore
            if ($store.profiles -and $store.profiles.PSObject.Properties[$state.ProfileId]) {
                $p = $store.profiles.($state.ProfileId)
                if ($state.ModsPath) { $p.modsPath = $state.ModsPath }
                $p.updatedAt = (Get-Date).ToString('o')
                $store.activeProfileId = $state.ProfileId
                Save-ModProfilesStore $store
            }
        }
        $skipIntro = if ($null -ne $state.SkipGiantsIntro) { [bool]$state.SkipGiantsIntro } else { [bool]$chkWizardSkipIntro.IsChecked }
    ...
    $btnNext.Add_Click({
        ...
        if ($state.Step -eq 2) {
            ...
            if ($state.ModsPath -and -not (Test-Path $state.ModsPath)) {
                $r = Show-SRConfirm "Folder ne postoji:`n$($state.ModsPath)`n`nNastaviti?" 'Konfiguriraj save' 'Da' 'Ne'
                if ($r -ne 'Yes') { return }
            }
        }
        if ($state.Step -eq 3) {
            $state.SkipGiantsIntro = [bool]$chkWizardSkipIntro.IsChecked
        }
        if ($state.Step -eq 4 -and $lstProfiles.SelectedItem) {
            $pi = $lstProfiles.SelectedItem
            $state.ProfileId = if ($pi.Id) { [string]$pi.Id } else { '' }
            $state.ProfileLabel = if ($pi.Label) { [string]$pi.Label } else { $state.ProfileId }
            if ($pi.Mods -and -not $state.ModsPath) { $state.ModsPath = [string]$pi.Mods; $txtMods.Text = $state.ModsPath }
        }
        if ($state.Step -ge 6) {
            try {
                & $applyWizard
...
function Refresh-ModList {
    ...
    foreach ($sm in $serverMods) {
        $dlUrl = ''
        try { if ($sm.Url) { $dlUrl = [string]$sm.Url } } catch {}
        $verLabel = '-'
        try {
        ...
        if ($isLocal) {
            ...
            if ($smSha) {
                ...
                if (-not $localSha) {
                    Write-Log "WARN SHA nije izracunljiv: $displayName ($($localMod.Name)) - fallback na velicinu"
                    if ($useSizeCheck -and $sm.Size -gt 0) {
                        if ([Math]::Abs($localMod.Length - [long]$sm.Size) -gt 1024) {
                            $isOutdated = $true
                            $statusDetail = 'Velicina ne odgovara serveru (SHA nije moguce provjeriti)'
                            $serverSizeText = "{0:N1} MB" -f ($sm.Size / 1MB)
                            Write-Log "ZASTARIO (velicina, bez SHA): $displayName"
                        }
                    }
...
$btnBrowseExe.Add_Click({
    ...
    if ($d.ShowDialog() -eq 'OK') {
        $txtGameExe.Text = $d.FileName
        Set-PlatformRadio (Get-GamePlatformFromPath $d.FileName)
    }
})

if ($btnAutoDetectGame) {
    $btnAutoDetectGame.Add_Click({
        $plat = Get-SelectedGamePlatform
        $found = Find-GamePathForPlatform -Platform $plat
        if ($found) {
            $txtGameExe.Text = $found
            $script:Config.gamePath = $found
            $script:Config | Add-Member -NotePropertyName gamePlatform -NotePropertyValue $plat -Force
            if (-not $txtDlcPath.Text) {
                $dlc = Find-DlcPath -ExePath $found
                if ($dlc) { $txtDlcPath.Text = $dlc }
            }
            Write-Log "Auto-detect ($plat): $found"
            try { Show-Toast "Pronadeno: $found" 'success' } catch {}
        } else {
            Show-SRDialog "FS25 nije pronadjen za platformu: $plat`nProvjeri instalaciju ili odaberi exe rucno." "Auto Detect" "Warning"
        }
    })
}

if ($btnBrowseSavegames) {
    $btnBrowseSavegames.Add_Click({
        $d = New-Object System.Windows.Forms.FolderBrowserDialog
        $d.Description = "Odaberi FarmingSimulator2025 folder (savegames)"
        if ($txtSavegamesPath.Text -and (Test-Path $txtSavegamesPath.Text)) { $d.SelectedPath = $txtSavegamesPath.Text }
        if ($d.ShowDialog() -eq 'OK') { $txtSavegamesPath.Text = $d.SelectedPath }
    })
}

if ($btnBrowseDlc) {
    $btnBrowseDlc.Add_Click({
        $d = New-Object System.Windows.Forms.FolderBrowserDialog
        $d.Description = "Odaberi DLC / data folder (opcionalno)"
        if ($txtDlcPath.Text -and (Test-Path $txtDlcPath.Text)) { $d.SelectedPath = $txtDlcPath.Text }
        if ($d.ShowDialog() -eq 'OK') { $txtDlcPath.Text = $d.SelectedPath }
    })
}
...
if ($script:Config.lastSync) { Update-DashboardSyncPanel }
...
} elseif ($effModsForCount -and (Test-Path $effModsForCount)) {
    @(Get-ChildItem $effModsForCount -Filter "*.zip" -ErrorAction SilentlyContinue).Count
} elseif ($script:Config.modsPath -and (Test-Path $script:Config.modsPath)) {
...
$btnOpenModsFolder.Add_Click({
    ...
    if ($modsPath -and (Test-Path $modsPath)) {
        ...
        Show-Toast "Mods folder nije postavljen (Postavke / Multiplayer folderi)" "warn"
    }
})
...
$ctxCopyDownloadLink = $window.FindName('ctxCopyDownloadLink')
if ($ctxCopyDownloadLink) {
    $ctxCopyDownloadLink.Add_Click({
        $sel = $lstMods.SelectedItem
        if (-not $sel) { return }
        $url = Get-ModDownloadUrl -ModItem $sel
        if ($url) {
            [System.Windows.Clipboard]::SetText($url)
            Show-Toast 'Download link kopiran' 'success' 2500
            Write-Log "Kopiran download URL: $url"
        } else {
            Show-Toast 'Nema URL za ovaj mod' 'warn'
        }
    })
}
